...
This public repository is the reproducibility/reference snapshot for the
published RFID method paper. It is derived from a larger private working/archive
repository, `RFID_method`, which contains the fuller development and manuscript
context.

In the private archive, this repository is linked as the `dist/` Git submodule:

```text
RFID_method/dist -> https://github.com/m-jenkin/subglacial_RFID.git
```

This repository should be updated only deliberately, when the public snapshot
itself needs correction or clarification.
